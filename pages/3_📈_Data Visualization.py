...
st.set_page_config(
    page_title="Le Pard | Visualization", page_icon="📈", layout="centered"
)

# Hero Section
st.markdown(
    "<h3 style='text-align: center;'>HR Analytics Visualizations</h3>",
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["Box Plot", "Density Plot", "Scatter Plot", "Heatmap"]
)
...
with tab3:
    ...
    scatter_plot_type = st.selectbox(
        "Choose Scatter Plot",
        [
    ...
    x_coord = sampled_df[selected_variable].min() + 0.05 * (
        sampled_df[selected_variable].max() - sampled_df[selected_variable].min()
    )
    y_coord = sampled_df["MonthlyIncome"].max() - 0.1 * (
        sampled_df["MonthlyIncome"].max() - sampled_df["MonthlyIncome"].min()
    )
    ...
    fig.add_annotation(
        ...
        x=x_coord,
        y=y_coord
        - 0.1 * (sampled_df["MonthlyIncome"].max() - sampled_df["MonthlyIncome"].min()),
        showarrow=False,
    )
...
with tab4:
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            tickvals=list(range(len(corr_matrix.columns))), ticktext=corr_matrix.columns
        ),
        yaxis=dict(
            tickvals=list(range(len(corr_matrix.columns))), ticktext=corr_matrix.columns
        ),
    ...
    )  # Use the full container width to center
